...
setDragSource : function () {
	...
	if ( Agua.hasApps("custom") == false )	return;

	var types = Agua.getAppTypes("custom");

	for ( var i = 0; i < types.length; i++ )
	{
		// GET APPLICATIONS
		var sourceArray = Agua.getAppsByType(types[i], "custom");
		...
		for ( var j = 0; j < sourceArray.length; j++ )
		{
			...
			dataArray.push( { item: data, type: ["draggableItem"] } );
		}
