dojo.provide("plugins.core.DropTarget");
...
dojo.declare("plugins.core.DropTarget",
	[ dojo.dnd.Target ], {
	...
	onDndDrop : function(source, nodes, copy)
	{
		...
		{ 
				...
				else
				{
					...
					if(copy)
					{
						...
						this._normalizedCreator = function(node, hint)
						{
							...
							// CLONE THE DROPPED NODE (WE ADD THE CLONE TO THE
							// SOURCE, *** NOT *** THE DROPPED NODE)
							var n = node.cloneNode(true);
							n.id = dojo.dnd.getUniqueId();
							var nodeId = n.id;

							// GET REQUIRED FIELDS FOR NEW STAGE
							var project = workflowObject.projectCombo.getValue();
							var workflow = workflowObject.workflowCombo.getValue();
							var number = 1;
							if ( this.current != null ) number = this.current.getAttribute('number') + 1;
							var name = node.application.name;

							// ADD STAGE TO Agua DATA AND REMOTE DATABASE
							workflowObject.addStage({ project: project, workflow: workflow, name: name, number: number });

							// THE CLONED NODE IS MISSING THE EXTRA METADATA
							// ATTRIBUTES WE ADDED TO THE ORIGINAL NODE,
							// E.G., THE APPLICATION DATA FOR THE APPLICATION
							// THE NODE REPRESENTS.
							// SO WE ADD THESE ATTRIBUTES TO THE CLONED NODE NOW.
							n.application = node.application;
							n.application.project = workflowObject.project;
							n.application.workflow = workflowObject.workflow;



							// ADD TARGET MENU
							workflowObject.targetMenu.bindDomNode(n);

							//// SET INFO PANE FOR DROPPED NODE
							workflowObject.setInfoPane(n);
			...
			// INSERT DROPPED NODES
			this.insertNodes(true, nodes, this.before, this.current);


			var allNodes = this.allNodes();

			//for ( var index = currentNumber + 2; index < elementArray.length ; index++ )
			//	//{
			//	//	elementArray[index].setAttribute('number') = elementArray[index].setAttribute('number') + 1;
			//	//}
