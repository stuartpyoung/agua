...
// CSS files
cssFiles : [
    "dojo-1.5.0/dojox/layout/resources/ExpandoPane.css",
...
// RETURN AN ARRAY NAMES OF WORKFLOWS IN this.workflows WITH THE SPECIFIED PROJECT
	...
	workflows = this.sortNumericHasharray(workflows, "workflownumber");	
...
// WORKFLOW NUMBER GIVEN PROJECT AND WORKFLOW IN this.workflows


	var workflowObjects = this._getWorkflow(projectName, workflowName);

	return workflowObjects[0].number;
},
...
// ADD AN EMPTY NEW WORKFLOW OBJECT TO A PROJECT OBJECT
    ...
    var workflows = this.filterByKeyValues(this.workflows, ["project"], [projectName]);
    var workflowNumber = 1;
    if ( workflows != null )
        workflowNumber = workflows.length + 1;
...
// OWNED BY ANOTHER USER
	...
	var params = dojo.clone(this.sharedparameters);
...
_getStageParameters : function () {
    if ( this.stageparameters == null ) return;
    return dojo.clone(this.stageparameters);
},
...
// REMOVE STAGE PARAMETERS FOR A STAGE FROM this.stageparameters 
	...
	var uniqueKeys = [ "project", "workflow", "appname", "appnumber", "name"];
	var removeOk = this._removeArrayFromArray(this.stageparameters, removeTheseStageParameters, uniqueKeys);
	if ( removeOk == false )
	{
		return false;
	}
...
// RETURN THE CLUSTER FOR THIS WORKFLOW, OR '' IF NO CLUSTER ASSIGNED
	...
	if ( clusterworkflows != null && clusterworkflows.length > 0 )
        cluster = clusterworkflows[0].cluster;
