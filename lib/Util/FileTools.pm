...
sub records {
    ...
    while ( <FILE> )
    {
		print "Util::FileTools::records    $records\n" if defined $dot and $records % $dot == 0;
		$records++;
    } 
	close(FILE);
...
sub peek {
	...
	print "Util::FileTools::peek    Util::FileTools::peek(args)\n";
	print "Util::FileTools::peek    args: \n";
	print Dumper $args;
	...
	if ( $inputfile =~ /^\s*$/)
	{
		print "Util::FileTools::peek    inputfile not specified\n";
		exit;
	}
...
#************************ USE awk /start/,/stop/ TO PRINT AN INTERVAL OF LINES
	...
	{	
		...
		return {error => 'Util::FileTools::_checkFile    fileroot not defined for user: $username and filepath: $filepath' } if not defined $fileroot or not $fileroot;
	...
	{
		print "Util::FileTools::peek    requestor $requestor cannot access project $project\n";
		exit;
	}
...
sub _peek {
	...
	print "Util::FileTools::_peek    action $action...\n";
	print "Util::FileTools::_peek    start $start...\n";
	...
	{
		return "{  error: 'Util::FileTools::_peek    Can't find inputfile found: $inputfile' }";
	}
	...
	print "Util::FileTools::_peek    line: $line\n";
	close(FILE);
	...
	if ( $action eq "head" or $action eq "tail" )
	{
		...
		{
			return "{  error: 'Util::FileTools::filepeek    peek at start $start and quantity $quantity not defined for inputfile: $inputfile' }";
		}		
		$peek =~ s/\n/\\n/g;
		$peek =~ s/'/\\'/;
	...
	{
		my $peek = '';
		open(FILE, $inputfile) or print "{ error: 'Util::FileTools::filePeek    Could not find file: $inputfile' }" and exit;
		seek FILE, $start, 0;
...
print "outputdir: $outputdir\n";
	...
	#### PRINT LINES INTO A SEPARATE FILE PER UNIQUE VALUE IN COLUMN
	print "Printing files with unique values in column $column...\n";
	open(FILE, $inputfile) or die "Can't open input file: $inputfile\n";
...
print "inputfile: $inputfile\n";
	...
	print "Util::FileTools::columnSplit    Can't create output sub-directory: $subdir_path\n" and exit if not -d $subdir_path;

	#### OPEN OUTPUT FILE AND PRINT FIRST LINE
	my $outfile = "$subdir_path/$outputfile";
	$outfile .= ".$suffix" if defined $suffix;
	print "Util::FileTools::columnSplit    outfile: $outfile\n";
	...
	while ( <FILE> )
	{
		...
		else
		{
			...
			print "Util::FileTools::columnSplit    Creating new sub directory: $subdir_path\n" if not -d $subdir_path;
...
sub columnAverage {
	...
	print "Util::FileTools::columnAverage    column not an integer: $column\n" and return if not $column =~ /^\d+$/;
...
sub columnSum {
	...
	#### CHECK COLUMN IS INTEGER VALUE
	print "Util::FileTools::columnSum    column not an integer: $column\n" and return if not $column =~ /^\d+$/;
	...
	while ( <FILE> )
	{
		...
		if ( $type =~ /^fasta$/i )
		{
			...
			if ( $number_reads == 0 )
			{
				...
				{
					print "Sequence length (", length($sequence), " is less desired length: $length. Exiting.\n";
					exit;
				}
			}
		...
		else
		{
			...
			if ( $number_reads == 0 )
			{
				...
				{
					print "Sequence length (", length($sequence), " and/or quality length (", length($quality), " is less than desired length: $length. Exiting.\n";
					exit;
				}
			}
			print OUTFILE substr($sequence, 0, $length), "\n";
...
#exit;
    ...
    foreach my $symbol ( @symbols )
    {
		...
		{
			print "ascii not defined for symbol $symbol_counter: **$symbol**\nin quality: ***$quality***\n";
			exit;
		}
...
print "Util::FileTools::sortFile(args)\n";	
#exit;
...
sub matesPaired {
	...
	{
		print "Util::FileTools::matesPaired    Number of read files (", scalar(@$inputfiles), " is not the same as the number of mate files (", scalar(@$matefiles), "). Exiting\n";
		exit;
	}
	...
	for ( my $i = 0; $i < @$inputfiles; $i++ )	
	{
		...
		{
			print "Util::FileTools::matesPaired    Expected matefile:\n$matefile\nBut found file:\n$$matefiles[$i]. Exiting\n";
			exit;
		}
	}
...
sub mergeFiles {
	...
	{
		print "Util::FileTools::mergeFiles inputfile not found: $file\n" and exit if not -f $file;
	}

	### MERGE IF MORE THAN ONE INPUT FILE
	if ( scalar(@$files) > 1 )
	{
		print "Util::FileTools::mergeFiles    Merging input files\n";
	...
	{
		print "Sole input file identical to output file: $outputfile\n" and exit if $$files[0] eq $outputfile;

		print "Util::FileTools::mergeFiles    Copying input files\n";
...
sub readQuality {
	...
	while ( <FILE> )
	{
		print "$number_reads\n" if $number_reads % 10000 == 0;
		my $header = $_;
		if ( not defined $header )
		{
			print "header not defined in read $number_reads\n";
			print "next line: <FILE>\n";
			$number_reads++;
			exit;
		}
		$header =~ s/\n$//;
...
sub trimRead {
	...
	print "Util::FileTools::trimRead    length not defined. Exiting\n" and exit if not defined $length;
...
sub svnBackup {
	...
	#### CHECK SOURCE DIR
	print "Can't find source directory: $sourcedir\n" and exit if not -d $sourcedir;

	#### CHECK REPOSITORY
	my $list_output = `svn list $repository`;
	print "Can't list repository: $repository" and exit if not defined $list_output or not $list_output;
	...
	chdir($workingdir_parent) or die "Util::FileTools::svnSync    Can't chdir to working directory parent: $workingdir_parent\n";
...
sub svnSync {
	...
	chdir($workingdir) or die "Util::FileTools::svnSync    Can't chdir to working directory: $workingdir\n";
...
sub svnAddDelete {
	...
	chdir($workingdir) or die "Util::FileTools::svnAddDelete    Can't chdir to working directory: $workingdir\n";
...
sub svnClean {
	...
	if ( not -d $directory )
	{
		print "Util::FileTools::svnClean    not a directory: $directory\n";
		return;
	}
	opendir(DIR,$directory) or die "Util::FileTools::svnClean    Can't open directory: $directory\n";
	@$files = readdir(DIR);
	close(DIR);
...
sub comment {
	...
	{
		print "Doing directory: $inputdir\n";
...
sub comment_action {
	...
	print "Finished outputfile: $inputfile\n";
