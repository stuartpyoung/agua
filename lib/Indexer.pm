...
sub buildIndex
{
    ...
    while ( <$input_filehandle> )
    {
		...
		print "Indexer::buildIndex    $counter\n" if defined $dot and $counter % $dot == 0;
		chomp($_);
		...
		if ( defined $regex )
		{
			...
			print "Can't get ID for line: $_\n" and exit if not defined $id;
...
sub location_size
{
	...
	if ( $type eq "read" )
	{
		...
		{
			print "Id done not have correct format ([^\-]+)\-([^\-]+)\-([^\-]+): $id \n";
