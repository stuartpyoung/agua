...
sub run {
	...
	if ( defined $loopout )
	{
		print "Logic::Loop::run    Printing STDOUT to loopout:\n\n$loopout\n\n";
	...
	#### DEBUG
	print "Logic::Loop::run     executable: $executable\n";
	print "Logic::Loop::run     parameter:  $parameter\n" if defined $parameter;
	print "Logic::Loop::run     values:     $values\n" if defined $values;
	print "Logic::Loop::run     replicates: $replicates\n" if defined $replicates;
	print "Logic::Loop::run     loop: $loop\n" if defined $loop;
	print "Logic::Loop::run     cluster: $cluster\n" if defined $cluster;
	print "Logic::Loop::run     outdir: $outdir\n" if defined $outdir;
	print "Logic::Loop::run     label: $label\n" if defined $label;

	#### CHECK loop IS DEFINED AND CORRECT VALUE
	print "Logic::Loop::run    loop not defined. Exiting\n" and exit if not defined $loop;
	if ( $loop eq "distributed" )
	{
		print "Logic::Loop::run    cluster not defined. Exiting\n" and exit if not defined $cluster;
		print "Logic::Loop::run    label not defined. Exiting\n" and exit if not defined $label;
		print "Logic::Loop::run    outdir not defined. Exiting\n" and exit if not defined $outdir;
		print "Logic::Loop::run    maxjobs not defined. Exiting\n" and exit if not defined $maxjobs;
		print "Logic::Loop::run    queue not defined. Exiting\n" and exit if not defined $queue;
	}

	print "Logic::Loop::run    loop value not supported: $loop\n" and exit
		if $loop !~ /^(serial|parallel|distributed)$/;

	print "Logic::Loop::run    Neither parameter nor replicates is defined. Exiting.\n" if not defined $parameter and not defined $replicates;
	print "Logic::Loop::run    values not defined for parameter. Exiting.\n" if defined $parameter and not defined $values;
...
#exit;
	...
	if ( $loop eq "distributed" )
	{
		print "Logic::Loop::run    Running loop in $loop mode\n";
		...
		if ( $cluster =~ /^PBS$/ )
		{
			print "Logic::Loop::run    DOING require Monitor::PBS\n";
			eval "require Monitor::PBS";
		}
		elsif ( $cluster =~ /^LSF$/ )
		{
			print "Logic::Loop::run    DOING require Monitor::LSF\n";
		...
		{
			print "Logic::Loop::run    cluster $cluster did not match LSF or PBS\n";
		}
		...
		print "Logic::Loop::run    Finished doing distributed jobs\n";
	}

	#### RUN COMMANDS IN PARALLEL LOCALLY
	elsif ( $loop eq "parallel" )
	{
		print "Logic::Loop::run    DOING CONCURRENT JOBS\n";
		my $threads = [];
		for my $command ( @$commands )
		{
			print "Logic::Loop::run    CONCURRENT: $command\n";
		...
		foreach my $thread ( @$threads )
		{
			...
			print "Logic::Loop::run    OUTPUT: $output\n";
			push @$outputs, $output;
		}
		...
		print "Logic::Loop::run    Finished doing parallel jobs\n";
	}

	#### RUN COMMANDS IN SERIES
	elsif ( $loop eq "serial" )
	{
		print "Logic::Loop::run    Running loop in 'serial' mode\n";

		#### RUN THE COMMANDS
		my $outputs = [];
		foreach my $command ( @$commands )
		{
			print "Logic::Loop::run     $command\n";
			my $output = `$command`;
			push @$outputs, $output;
		}
	...
	{
		print "Logic::Loop::run    Exiting. Missing handler for loop mode: $loop\n";
		exit;
	}
	...
	if ( defined $oldout )
	{
		print "Logic::Loop::run    Redirecting STDOUT back to standard output\n";
		open STDOUT, ">&", $oldout;
	}
...
sub loop {
	...
	print "Logic::Loop::run     executable: $executable\n";
	print "Logic::Loop::run     arguments:  @$arguments\n";
	print "Logic::Loop::run     parameter:  $parameter\n" if defined $parameter;
	print "Logic::Loop::run     values:     @$values\n" if defined $values;
	print "Logic::Loop::run     replicates: @$replicates\n" if defined $replicates;
