...
sub usageStats {
	...
	{
		print "Agua::Cluster::Usage::usageStats    Checking non-batch stdout file\n";
		my $usage = [];
		foreach my $job ( @$jobs )
		{
	...
	{
		print "Agua::Cluster::Usage::usageStats    Checking batch stdout files\n";
		foreach my $job ( @$jobs )
		{
			my $tasks = $job->{tasks};
...
sub parseUsagefile {
	...
	print "Agua::Cluster::Usage::parseUsagefile    usage file not defined. Returning\n" and return if not defined $file;
	...
	print "Agua::Cluster::Usage::parseUsagefile    Can't open output file: $file\n" and return if not $success;	
	$/ = "END OF FILE";
	my $output = <$fh>;
	close($fh);
	$/ = $temp;
...
sub printUsage {
	...
	print "Agua::Cluster::Usage::printUsage    previous usagefile PRESENT!\n" if -f $usagefile;
	...
	print "Agua::Cluster::Usage::printUsage    batchstats not defined. Returning\n" and return if not defined $batchstats;	
	print "Agua::Cluster::Usage::printUsage    Printing USAGE file: $usagefile\n";
	...
	print "Agua::Cluster::Usage::printUsage    starttime: $starttime\n";
	my $duration;
