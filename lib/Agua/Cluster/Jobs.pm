...
has 'conf'		=> ( isa => 'HashRef|Undef', is => 'rw' );
...
sub moveToDir {
	...
	print "Agua::Cluster::Jobs::moveToDir    sourcedir is a file: $sourcedir\n" and return if -f $sourcedir;
	print "Agua::Cluster::Jobs::moveToDir    targetdir is a file: $targetdir\n" and return if -f $targetdir;
	print "Agua::Cluster::Jobs::moveToDir    Can't find sourcedir: $sourcedir\n" and return if not -d $sourcedir;

	#### CREATE TARGET DIR IF NOT EXISTS
	File::Path::mkpath($targetdir) if not -d $targetdir;
	print "Skipping move because can't create targetdir: $targetdir\n" if not -d $targetdir;
...
sub runJobs {
	...
	print "Agua::Cluster::Jobs::runJobs    jobs not defined\n"
		and exit if not defined $jobs;
	print "Agua::Cluster::Jobs::runJobs    label not defined\n"
	...
	#{
		print "Agua::Cluster::Jobs::runJobs    doing check $counter with checkStatus(jobs, label)\n";
		$counter++;
		($status, $sublabels) = $self->checkStatus($jobs, $label);
		print "Agua::Cluster::Jobs::runJobs    status: $status\n";
		print "Agua::Cluster::Jobs::runJobs    Sleeping for 10 seconds\n" if not $status;
	#	sleep(10) if not $status;
	#}
	print "Agua::Cluster::Jobs::runJobs    Final value of status: $status\n";
	...
	#### PRINT DURATION
	print "Agua::Cluster::Jobs::runJobs    Completed $label at ", Timer::current_datetime(), ", duration: $duration\n";
		...
		and $self->cluster() eq "LSF" )
	{
		...
		print "Agua::Cluster::Jobs::runJobs    Doing printUsage       ", Timer::current_datetime(), "\n";
		$self->printUsage($jobs, $label);
		print "Agua::Cluster::Jobs::runJobs    After printUsage       ", Timer::current_datetime(), "\n";
	}
	...
	{
		print "Agua::Cluster::Jobs::runJobs    Cleaning up scriptfiles.\n";
		foreach my $scriptfile ( @$scriptfiles )
		{
			print "Agua::Cluster::Jobs::runJobs    Removing scriptfile: $scriptfile\n";
			`rm -fr $scriptfile*`;
		}
	}
	else
	{
		print "Agua::Cluster::Jobs::runJobs    cleanup not defined. Leaving scriptfiles.\n";
	}
...
sub executeLocal {
	...
	print "Agua::Cluster::Jobs::executeLocal    jobs not defined\n" and exit if not defined $jobs;
	print "Agua::Cluster::Jobs::executeLocal    label not defined\n" and exit if not defined $label;

	#### INPUTS
	my $monitor = $self->monitor();		#### ACCESSOR IS IMPLEMENTED
	my $cluster = $self->cluster();
	...
	print "Agua::Cluster::Jobs::executeLocal    maxjobs not defined. Exiting\n" and exit if not defined $maxjobs;

	my $jobids = [];
	my $scriptfiles = [];

	my $counter = 0;
	print "Agua::Cluster::Jobs::executeLocal    executing ", scalar(@$jobs), " jobs\n";
	foreach my $job ( @$jobs )
	{	
		$counter++;
		print "Agua::Cluster::Jobs::executeLocal    counter: $counter\n";
		#### CREATE OUTPUT DIRECTORY
		my $outputdir = $job->{outputdir};
		File::Path::mkpath($outputdir) if not -d $outputdir;
		print "Agua::Cluster::Jobs::executeLocal    Can't create outputdir: $outputdir\n" and die if not -d $outputdir;

		#### MOVE TO OUTPUT DIRECTORY
		chdir($outputdir) or die "Can't move to output subdir: $outputdir/$counter\n";	

		print "Agua::Cluster::Jobs::executeLocal    cluster not defined. executening commands locally\n";
		...
		{
			print "Agua::Cluster::Jobs::executeLocal    command: $command\n";
			print `$command`;
			print "Agua::Cluster::Jobs::executeLocal    Completed command\n";
		}
...
sub executeCluster {
	...
	print "Agua::Cluster::Jobs::executeCluster    jobs not defined\n" and exit if not defined $jobs;
	print "Agua::Cluster::Jobs::executeCluster    label not defined\n" and exit if not defined $label;

	#### INPUTS
	my $monitor = $self->setMonitor();		#### ACCESSOR IS IMPLEMENTED
	my $cluster = $self->cluster();
	my $maxjobs = $self->maxjobs();
	my $qsub 	= $self->qsub();
	my $qstat 	= $self->qstat();
	...
	#### QUIT IF maxjobs NOT DEFINED
	print "Agua::Cluster::Jobs::executeCluster    maxjobs not defined. Exiting\n" and exit if not defined $maxjobs;
	...
	if ( $execute )
	{
		#### execute EVERY JOB
		my $counter = 0;
		print "Agua::Cluster::Jobs::executeCluster    executing ", scalar(@$jobs), " jobs\n";
		foreach my $job ( @$jobs )
		{	
			$counter++;
			...
			my $outputdir = $job->{outputdir};
			File::Path::mkpath($outputdir) if not -d $outputdir;
			print "Agua::Cluster::Jobs::executeCluster    Can't create outputdir: $outputdir\n" and die if not -d $outputdir;
			...
			if ( not defined $cluster )
			{
				print "Agua::Cluster::Jobs::executeCluster    cluster not defined. executening commands locally\n";
				my $commands = $job->{commands};
				...
				if ( defined $stdoutfile )
				{
					print "Agua::Cluster::Jobs::executeCluster    diverting STDOUT to: $stdoutfile\n";
					open my $oldout, ">&STDOUT"  or die "Can't duplicate STDOUT: $!";
					open my $olderr, ">&STDERR"  or die "Can't duplicate STDERR: $!";
					#open OLDERR,     ">&", \*STDERR or die "Can't dup STDERR: $!";					
					open(STDOUT, ">$stdoutfile") or die "Can't redirect STDOUT to file: $stdoutfile" if defined $stdoutfile;
				...
				foreach my $command ( @$commands )
				{
					print "Agua::Cluster::Jobs::executeCluster    command: $command\n";
					print `$command`;
					print "Agua::Cluster::Jobs::executeCluster    Completed command\n";
				}
				print `unlink $lockfile`;
			...
			else
			{
				...
				print "Agua::Cluster::Jobs::executeCluster    jobid not defined. Exiting\n" and exit if not defined $jobid;
...
sub setJob {
	...
	$stdoutfile = "$stdoutdir/$label-stdout.txt" if not defined $stdoutfile;
	$stderrfile = "$stdoutdir/$label-stderr.txt" if not defined $stderrfile;
	$lockfile = "$lockdir/$label-lock.txt" if not defined $lockfile;


	#### SANITY CHECK
	print "Agua::Cluster::Jobs::setJob    commands not defined\n" and exit if not defined $commands;
	print "Agua::Cluster::Jobs::setJob    label not defined\n" and exit if not defined $label;
	print "Agua::Cluster::Jobs::setJob    outputdir not defined\n" and exit if not defined $outputdir;
...
sub setBatchJob {
	...
	print "Agua::Cluster::Jobs::setBatchJob    commands not defined. Exiting...\n" and exit if not defined $commands;
	print "Agua::Cluster::Jobs::setBatchJob    label not defined. Exiting...\n" and exit if not defined $label;
	print "Agua::Cluster::Jobs::setBatchJob    outputdir not defined. Exiting...\n" and exit if not defined $outputdir;
	print "Agua::Cluster::Jobs::setBatchJob    number not defined. Exiting...\n" and exit if not defined $number;
	...
	#### SET BATCH
	$job->{batch} = "$label\[1-$number\]" if $cluster eq "LSF";
	$job->{batch} = "-t $number" if $cluster eq "PBS";
	$job->{batch} = "-t 1-$number" if $cluster eq "SGE";
...
sub printLsfScriptfile {
	...
	#### SANITY CHECK
	print "Agua::Cluster::Jobs::printLsfScriptfile    scriptfile not defined\n" and exit if not defined $scriptfile;
	print "Agua::Cluster::Jobs::printLsfScriptfile    commands not defined\n" and exit if not defined $commands;
	print "Agua::Cluster::Jobs::printLsfScriptfile    label not defined\n" and exit if not defined $label;
