...
sub filePeek
{
	...
	elsif ( defined $bytes and defined $offset )
	{
		my $peek = '';
		$peek =~ s/'/\\'/;
		open(FILE, $filepath) or print "{ error: 'FileManager::filePeek    Could not find file: $filepath' }" and exit;
		seek FILE, $offset, 0;
...
sub checkFiles
{
    ...
    ### VALIDATE    
    print "{ error: 'FileManager::checkFiles    User session not validated' }" and exit unless $self->validate();
